<x-app-layout>
    ...
    <div class="container mx-auto mt-10 p-4 pb-40 max-w-lg min-h-screen">
        ...
        <form action="/posts" method="POST" class="space-y-5" enctype="multipart/form-data">
            @csrf
            ...
            <div>
                ...
                <input type="file" name="image" id="image" class="mt-1 w-full rounded-md border-gray-200 bg-white text-sm text-gray-700 shadow-sm focus:border-gray-400 focus:ring-0 outline-none p-2 border @error('image') border-red-500 @enderror">
                @error('image')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
